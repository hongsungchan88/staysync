...
요청 한도 대응이 설계에 반드시 반영되어야 한다. Channex는 분당 총 20건, 숙소당 분당 10건(요금·제약), 숙소당 분당 10건(재고)으로 제한하며 초과 시 429를 반환한다. 문서는 변경사항을 모아 6초마다 한 번 호출하는 방식을 권장하며, 429를 받으면 해당 숙소의 전송을 1분 멈춘 뒤 지수 백오프로 재시도할 것을 권한다.
...
변경 병합과 날짜 구간 압축을 함께 적용하면, 캘린더에서 30일을 드래그해 요금을 바꿔도 채널당 API 호출이 1회로 끝난다. 순진하게 구현할 경우 30일 × 3채널 = 90회 호출이 발생해 요청 한도에 걸린다. Channex는 요청 본문을 10MB까지 받으므로 실제 목표는 더 높게 잡는다. 자가 인증 항목이 6개월치 요금과 제약을 호출 1회로 전송할 것을 요구하므로 압축 기준을 그쪽에 맞춘다.

웹훅은 서명을 검증한 뒤 202를 즉시 반환하고, 실제 처리는 동기화 큐에 적재해 워커가 수행한다. 웹훅 핸들러에서 동기 처리를 하면 채널 측 타임아웃과 재전송을 유발한다.

#### 개발과 검증 환경

Channex는 `staging.channex.io`에 무료 스테이징 서버를 운영한다. 구독 없이 API 키가 발급되고 카드 등록이나 영업 상담을 요구하지 않는다. 인증은 `user-api-key` 헤더로 하며 키는 숙소 단위로 범위를 제한할 수 있다.

스테이징에서 붙일 수 있는 채널은 두 가지다. 부킹닷컴은 공용 테스트 숙소가 미리 준비되어 있어 별도 승인 없이 연결한다. 통화가 GBP, EUR, USD, JPY로 나뉘어 있으며 요금제 통화를 숙소와 맞춰야 매핑이 되고, 객실과 요금제를 모두 매핑해야 채널이 활성화된다. 익스트라넷 접근은 없고 API로만 다룬다. 에어비앤비는 별도 샌드박스가 없어 실제 호스트 계정을 연결하는 방식이며, 가짜 리스팅 생성은 계정 차단 사유가 될 수 있다고 문서가 경고한다.

이는 10.3절이 "파트너 승인 수개월"로 분류한 부킹닷컴 공식 연동 관문을 지나지 않고도, Mock이 아닌 실제 채널을 상대로 ARI 전송과 예약 수신을 검증할 수 있다는 뜻이다. 3.3절 채널 연동 전략의 근거가 이 사실 위에서 강해진다.

#### 자가 인증 항목

프로덕션 전환에는 Channex가 정한 자가 인증 14항목 통과가 필요하며, 시험은 스테이징에서 수행한다. 프로덕션에 가지 않더라도 이 목록은 P3의 완료 조건으로 쓸 수 있다. 자체 기준이 아니라 상용 채널 매니저가 정한 기준이라는 점에서 평가에서 방어하기 쉽다.

항목은 전체 동기화(500일치를 호출 2회), 단일·복수 날짜와 요금 조합의 묶음 전송, 최소 숙박일과 판매 중지를 비롯한 제약 전송, 6개월치 일괄 전송, 재고 변경, 예약 수신·수정·취소와 수신 확인, 요청 한도 준수, 델타 전송, 지원 기능 선언으로 구성된다.

이 가운데 요청 한도 준수와 델타 전송 두 항목이 6.5절의 ARI 병합 버퍼와 6.6절의 정기 재동기화 설계가 겨냥하는 지점이다. 전체 재동기화 타이머가 아니라 변경이 전송을 유발해야 한다는 요구가 6.6절의 설계 방향과 일치한다.
...
멱등성 키는 **대기·실행 중인 작업 안에서만** 유일해야 한다. 전 구간에 유일 제약을 걸면 같은 값을 다시 보내는 일이 영영 막힌다. 요금을 25만 → 23만 → 25만으로 되돌리면 마지막 전송이 중복으로 걸러져 채널에는 23만이 남고, 우리 쪽 로그는 깨끗하다.
...
위 스케치의 `VectorStore` 자리는 P5 17주차 착수 시점에 자체 포트(`VectorSearchPort`)로 바꿔 잡았다. 이유는 실행 환경이 둘로 갈리기 때문이다. 개발용 `local` 프로파일은 내장 PostgreSQL 바이너리를 쓰는데 여기에는 pgvector 확장이 없어 `PgVectorStore`를 띄울 수 없고, 대안인 `SimpleVectorStore`는 벡터를 메모리에 따로 들고 있어 청크는 데이터베이스에 있는데 검색이 보는 것은 메모리에만 있는 이원 상태가 된다. 재기동하면 사라진다. 어느 쪽이든 구현이 둘로 갈리므로 그 지점에서 Spring AI의 추상화가 벌어 주는 것이 없다.

또한 `PgVectorStore`는 자기 스키마를 직접 관리해 `vector_store` 테이블을 따로 만든다. 5.3의 `knowledge_chunk`가 이미 `embedding` 컬럼을 들고 있고 스키마는 Flyway가 관리한다는 원칙이 있으므로, 같은 데이터를 두 테이블에 두는 결과가 된다.

그래서 임베딩 생성만 Spring AI의 `EmbeddingModel`을 사용하고 저장과 검색은 `knowledge_chunk`에 둔다. 포트 하나에 구현 둘을 두어, pgvector를 쓸 수 있는 환경에서는 코사인 거리 연산자와 HNSW 인덱스로 질의하고 그렇지 않은 환경에서는 `REAL[]` 컬럼을 읽어 애플리케이션에서 코사인 유사도를 계산한다. 두 구현이 같은 데이터에 같은 순위를 내는 것을 테스트로 고정한다. 근거와 완료 조건은 `docs/작업지시-14-지식베이스와검색.md` 5절 3번에 있다.

키워드 축도 같은 제약을 받는다. 인덱스는 `to_tsvector('simple', content)`인데 `simple` 설정은 공백과 문장부호로만 자르고 어간 분석을 하지 않으므로, 조사가 어절에 붙는 한국어에서는 문서의 "체크인은"과 질의의 "체크인"이 서로 다른 토큰이 된다. 형태소 분석기나 n-gram 확장은 `local` 환경에서 사용할 수 없다. 질의 토큰에 접두 매칭을 걸고 흔한 조사를 떼는 방식으로 처리하며, 이것이 실제로 동작하는지를 완료 조건으로 고정한다. 이 축이 동작하지 않아도 병합 결과는 벡터 축의 결과를 돌려주므로 화면에는 증상이 나타나지 않기 때문이다.
...
| 서비스 | 용도 | 비용 | 확보 난이도 | 비고 |
|---|---|---|---|---|
| Channex 스테이징 | 다채널 연동 개발·검증 | 무료 | 낮음 | 가입 즉시 API 키 발급, 구독·카드 불필요. 부킹닷컴 테스트 숙소 제공 |
...
| 서비스 | 용도 | 비용 | 확보 난이도 | 비고 |
|---|---|---|---|---|
| Channex 프로덕션 | 실제 예약 트래픽 연동 | 월 $130 + 유닛당 $0.5 | 보통 | 자가 인증 14항목 통과 필요. 최소 계약 없음, 최종 시연 1개월만 구독 검토 |
...
Channex 스테이징은 문의나 승인 절차가 없다. 2026년 8월 24일 조사에서 확인한 바로는 `staging.channex.io`에 가입하면 구독 없이 API 키가 발급되며 카드 등록도 영업 상담도 요구하지 않는다. 따라서 12주차까지 미룰 이유가 없고 P1 기간에 계정과 키를 확보해 접근 가능성을 먼저 없애 둔다. 상세는 `docs/조사-01-channex-샌드박스.md`에 있다.

프로덕션 전환은 별개의 판단이다. 구독만으로 되지 않고 자가 인증 14항목을 스테이징에서 통과해야 하므로, 최종 시연에 프로덕션이 필요한지 여부는 P3 결과를 보고 결정한다.
...
| P4 운영·부가 | 11/17~12/07 | 14–16 | 인박스, 태스크, 리포트, 직접예약 | 기능 완성 |
| P5 AI | 12/08~01/04 | 17–20 | 게스트 응대, 요금 추천 | AI 평가 결과 |
...
완료 조건은 브라우저에서 수기 예약을 만들면 캘린더에 즉시 표시되고 30일 요금을 한 번에 변경할 수 있는 상태다. 이 시점에 지도교수 중간 점검을 요청한다. 이 시점에 계획서의 진척 현황과 발표자료를 갱신한다.
...
완료 조건은 세 가지다. 첫째, Mock 채널에서 발생한 예약이 15초 이내에 캘린더에 반영되고 다른 채널의 재고가 자동으로 차감된다. 둘째, 5% 에러율을 주입한 상태에서 ARI 100건을 전송해도 재시도 후 최종 정합성이 유지된다. 셋째, 실제 에어비앤비 리스팅이 발행한 iCal 피드를 수신해 캘린더에 반영하고, StaySync가 발행한 iCal을 에어비앤비가 실제로 읽어 간다.

셋째 조건은 초안에서 "실제 에어비앤비 리스팅의 예약이 캘린더에 나타난다"였다. 실제 계정으로 확인한 결과 미게시 리스팅은 전 기간을 하나의 차단 일정으로 내보내 개별 예약을 구분할 수 없고, 게시하면 검색에 노출되어 실제 게스트가 예약하고 결제까지 일어날 수 있다. 학기 프로젝트의 테스트 리스팅에 감당할 위험이 아니라고 보아 미게시를 유지하기로 했다. 예약 수신 시나리오는 Channex 스테이징의 부킹닷컴 테스트 숙소와 6.4의 Mock OTA 시뮬레이터가 맡는다. 근거와 iCal 파서 명세는 `docs/조사-02-에어비앤비-ical.md`에 있다.

셋째 조건의 발행 방향은 9월 7일 확인에서 한 번 더 좁혔다. 그전 문구는 "에어비앤비가 읽어 해당 날짜를 막는다"였으나, 읽은 결과가 에어비앤비 달력에 반영되는지는 이 리스팅으로 판정할 수 없다. 리스팅을 게시하려면 대한민국 숙박업 영업신고증이 필요하고, 미게시 리스팅은 전 기간이 이미 차단 상태여서 열린 날짜가 없다. 날짜가 닫히는 변화를 볼 자리가 없으며, 에어비앤비는 날짜 상세에 차단의 출처를 표시하지 않아 우리 달력 때문인지 미게시 때문인지도 구분되지 않는다. 앞 문단이 위험을 이유로 선택한 미게시 유지가 여기서는 제약으로 작용한다. 영업신고를 하지 않는 한 학기 안에 풀리지 않는다.

그래서 판정할 수 있는 지점까지로 조건을 다시 적었다. 에어비앤비 서버가 우리 발행 URL로 실제 요청을 보내 피드를 읽어 갔다는 것은 관측된다. 9월 7일 확인에서 공개 터널로 발행 URL을 열고 에어비앤비에 등록하자 에어비앤비 화면에 캘린더 이름과 최종 수정 시각이 표시됐다. 그 뒤의 달력 반영은 게시된 리스팅이 있어야 볼 수 있으므로 완료 조건에서 뺀다. 대신 발행물 자체가 재고 상태와 맞는지를 `IcalExportApiTest`와 9월 7일의 실물 대조로 확인한다. 구현의 결함이 아니라 검증 환경의 한계이며, 판정 근거는 `docs/확인-05-14주차-브라우저검증.md` 2절에 있다.

#### P4. 운영·부가 (14–16주차, 60시간)

14주차에 통합 인박스, 메시지 템플릿, 자동 발송 스케줄러를 구현한다. 15주차에 청소·태스크 자동 생성과 칸반, 알림을 만든다. 16주차에 리포트와 직접예약 위젯, 포트원 테스트 결제를 붙인다.

#### P5. AI (17–20주차, 80시간)

17주차에 지식베이스 CRUD와 청킹·임베딩 파이프라인, pgvector 하이브리드 검색을 만든다. 18주차에 안전 분류기, 프롬프트, 답변 생성, 인용 검증, 발송 정책을 구현한다. 19주차에 평가 집합 100문항과 평가 도구를 만들어 지표를 측정한다. 20주차에 요금 추천 엔진과 설명 생성, 승인 화면을 구현한다.

완료 조건은 자동 종결률 70% 이상, 근거 일치도 95% 이상의 측정 결과를 확보하는 것이다.

지도교수가 "기존 서비스들의 기능을 먼저 갖추고, 차별점으로 AI를 추가하라"고 제시해 초안의 AI ↔ 운영·부가 순서를 맞바꿨다. 기술적 의존 방향도 같은 쪽을 가리킨다. 게스트 자동 응대는 통합 인박스 위에 얹히는 기능이라, 초안 순서로는 응대할 화면이 없는 상태에서 응대 로직을 만들고 그다음에 화면을 만들게 된다. 각 단계의 주차 수와 시간은 그대로 옮겼다.

바뀐 순서의 대가는 AI 평가 지표가 마지막 단계에 놓인다는 것이다. 자동 종결률 70%에 미달하면 P6 안에서 고쳐야 하고 여유가 없다. 그래서 19주차의 평가 집합 100문항은 뒤로 미루지 않는다. 지표를 20주차에 처음 재면 손쓸 시간이 남지 않는다.
...
P4와 P5를 맞바꾸면서 1번과 3번, 4번이 전부 앞당겨졌다. 지연이 났을 때 자를 대상이 이미 만들어져 있을 수 있다는 뜻이다. 그때는 만들어 둔 것을 지우지 말고 2번(요금 추천의 LLM 설명부)부터 자른다. 이 순서는 무엇을 포기할지의 우선순위이지 착수 시점의 순서가 아니다.
...
**위 스케치의 `try { reserve } catch { forceBook }` 형태는 그대로 구현하면 동작하지 않는다.** P3 12주차 구현에서 확인한 것이라 여기 남긴다. `InventoryLedgerWriter.apply`가 `@Transactional`이므로 `InsufficientInventoryException`이 그 경계를 넘는 순간 바깥 트랜잭션이 rollback-only로 표시된다. 예외를 잡아 `forceBook`을 호출해도 커밋이 막혀 예약도 충돌 기록도 남지 않는다. **결정이 조용히 뒤집히는데 로그는 깨끗하다** — 거절했다는 흔적조차 남지 않는다.

실제 구현은 예외를 잡는 대신 재고를 먼저 조회해 분기한다. 이미 판매 단위 락을 쥔 상태라 조회와 쓰기 사이에 다른 요청이 끼어들지 않으므로 안전하다. 예외로 흐름을 제어하려면 별도 트랜잭션(`REQUIRES_NEW`)이 필요한데, 그러면 예약과 충돌 기록이 서로 다른 트랜잭션에 놓여 함께 롤백되어야 한다는 성질이 깨진다. 4.4의 이벤트 기록, 15.1의 감사 로그와 같은 판별 기준이다.
...
여기에 조건이 하나 더 필요하다. 같은 연결에 앞선 작업이 아직 끝나지 않았으면 뒤엣것을 집지 않는다(`NOT EXISTS`). 이 조건이 없으면 앞선 작업이 백오프로 밀린 사이 뒤엣것이 먼저 나가고, 나중에 재시도된 옛 값이 새 값을 덮어쓴다. P3 완료 조건의 "최종 정합성"이 이 조건 위에 선다. 대가는 맨 앞 작업이 막히면 그 연결 전체가 기다린다는 것이며, 8회 상한과 `DEAD` 처리가 그 대기를 끊는다.
...
| 직접예약 위젯 | 서버 재계산으로 금액·가용을 대조. **속도 제한은 미조치**(아래) |

**미조치 항목 — 공개 HOLD 경로의 속도 제한.** `POST /public/booking/{id}/hold` 는 이
시스템의 첫 공개 쓰기 경로다. 로그인 없이 재고를 15분씩 점유할 수 있고 결제까지 가지
않아도 되므로, 반복 호출로 그 숙소의 판매 가능 날짜를 비워 둘 수 있다.

P4 16주차에 3주차의 로그인 시도 제한을 재사용하려 했으나 재사용이 되지 않았다.
`LoginAttemptLimiter` 는 identity 모듈의 내부 타입이라 booking 이 참조할 수 없고,
성격도 다르다 — **실패를 세어 잠그는 장치**여서 성공한 요청에 겨누면 정상 이용자가
잠긴다. 상한과 창이 로그인과 공유된 상수인 것도 걸린다.

속도 제한기를 새로 만드는 것은 계획서 12.4 의 축소 순서 1번 기능에 인프라를 더하는
일이라 하지 않았다. **알고 남긴 구멍이다.** 실제 서비스로 올릴 때 반드시 채운다.

덧붙여, 넣었더라도 IP 는 바꿀 수 있으므로 완전한 방어는 아니다. 근본적으로는 결제
전 점유 자체를 짧게 하거나 캡차 같은 다른 층이 필요하다.
...
마스킹이 필요해지는 지점을 P5 17주차 착수 시점에 명확히 했다. 17주차의 지식베이스 인덱싱에서 외부로 나가는 것은 호스트가 직접 작성한 숙소 안내문뿐이며 게스트가 작성한 글은 포함되지 않는다. 게스트 메시지가 외부로 나가기 시작하는 것은 답변 생성을 구현하는 18주차이고, 마스킹은 그 지점에 둔다. 이 경계를 문서와 코드 주석에 남기는 이유는, 17주차에 임베딩이 이미 외부 API를 사용하고 있다는 사실 때문에 18주차에 마스킹 지점을 지나칠 수 있기 때문이다. 안내문에 게스트 연락처를 기재하지 않도록 지식베이스 편집 화면에 안내를 둔다.
