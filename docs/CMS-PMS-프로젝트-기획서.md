...
완료 조건은 세 가지다. 첫째, Mock 채널에서 발생한 예약이 15초 이내에 캘린더에 반영되고 다른 채널의 재고가 자동으로 차감된다. 둘째, 5% 에러율을 주입한 상태에서 ARI 100건을 전송해도 재시도 후 최종 정합성이 유지된다. 셋째, 실제 에어비앤비 리스팅이 발행한 iCal 피드를 수신해 캘린더에 반영하고, StaySync가 발행한 iCal을 에어비앤비가 읽어 해당 날짜를 막는다.
