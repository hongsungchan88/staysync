...
**4계층이 P3 12주차부터 실제로 채워진다.** 채널 수신이 재고를 넘겨 받아들이면
`overbooking_conflict` 에 날짜마다 한 행이 남고, 캘린더 셀의 `conflict` 가 그 행을 읽어
`true` 가 된다. 7주차에 항상 `false` 로 두고 "P3 에서 채운다"고 적어 둔 자리다.
해소 절차 넷과 관리 화면은 13주차다(계획서 7.4).
...
**다음** — P3 13주차. iCal 어댑터(수신·발행), 재동기화 배치, 충돌 해소 절차와 화면.
**조건 1의 여유가 6.3초다**(측정 8.7초 / 상한 15초, 최악 13초). 폴링 주기와 재동기화
설계가 그 안에 들어가야 한다. 지도교수 중간 점검, 계획서 진척 현황과 발표자료 갱신은
Cowork 세션이 한다.
...
**아직 비어 있는 모듈** — messaging, ops, payment, ai, analytics. channel은 12주차에
어댑터 하나와 워커까지 찼고 iCal 과 재동기화가 남았다. 각 패키지의 `package-info.java`에
담당 범위와 착수 시점을 적어뒀다.
