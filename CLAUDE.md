...
측정 결과는 `docs/측정-02-가상스크롤-이후.md`에 있다. DOM 노드 11,229 → 1,234개,
강제 레이아웃 130 → 1.2ms. **페인트를 포함한 브라우저 사이클은 못 쟀다** —
측정에 쓴 브라우저 페인이 합성을 하지 않는 상태였다. 눈에 보이는 브라우저에서 다시 재야
7주차의 996ms와 비교할 수 있다.
