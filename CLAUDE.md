...
Zustand · Tailwind CSS 4 · Zod · Vitest 3.
...
최소 구현, 프론트엔드 셋업과 캘린더 그리드. `clean-on-start`를 껐다. ADR 9건,
백엔드 테스트 111건 + 프론트 5건 통과.

**다음** — P2 8주차. 가상 스크롤, 예약 막대 드래그 이동, 기간 선택.
가상 스크롤의 근거는 `docs/측정-01-캘린더-초기렌더링.md`에 있다 — 2,700셀 사이클
996ms 중 73%가 프론트엔드이고 DOM 노드가 11,229개다. 9주차는 요금·제약 일괄 편집과
SSE다. 이 시점에 계획서 진척 현황과 발표자료를 갱신한다.

와이어프레임 미결 세 가지(그룹, 날짜 칸 너비, 막대 쌓기)는 실제 데이터를 올려 보고
정하기로 했다. 7주차에 임의로 정하지 않았다.
