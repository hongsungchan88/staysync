...
인터페이스, ADR 7건. P1 3주차: JWT 인증(가입·로그인·갱신·로그아웃·시도 제한),
리프레시 토큰 회전과 재사용 탐지, 숙소·판매 단위·요금제 REST API와 조직 스코핑.
P1 4주차: 예약 상태 머신과 재고 연동, 수기 예약, HOLD 만료 배치, 게스트 연락처 암호화,
예약 REST API. 테스트 75건 통과.

**다음** — P1 5~6주차. 재고 원장 보강, Outbox 패턴, 감사 로그.
`InventoryConcurrencyTest`가 P1의 완료 조건이자 평가 근거다.
역할별 인가 규칙(`@PreAuthorize`)은 아직 없다. 지금은 인증 여부와 조직 스코핑까지다.
채널 수신(`BookingIngestService`)은 P3다. `fromChannel`과 `applyRevision`은 자리만 있다.
